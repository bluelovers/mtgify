...
const { parseScryfallCardTypeExtra, getCardMainTypes } = require('./util/GroupKeyByCardTypePriority');
...
class CardAPI {
  ...
  async getCardInfoFallback(lookup, cardName, config) {
    const name = this.normalizeCardName(cardName, config)
    let card = this.fallbackCache[name]
    if (typeof card === 'undefined') {
      card = await window.fetch(`https://api.scryfall.com/cards/named?exact=${cardName}`, { cache: "no-store" })
        .then(res => res.json())
        .then(sf => {
          if (sf.object !== 'card' || !sf.set) {
            throw new Error()
          }
          return lookup.cards[name] = {
            name: sf.name,
            setCode: sf.set,
            cardNum: sf.collector_number,
            mid: sf.multiverse_ids && sf.multiverse_ids[0],
            types: getCardMainTypes(parseScryfallCardTypeExtra(sf.type_line)),
            cmc: sf.cmc,
          }
        })
        .catch(e => {
  ...
  getCardInfo(cardName, customConfig) {
    ...
    return self.lookupPromise
      .then(lookup => {
        let card = self.performLookup(lookup, cardName, config)
        return card !== null ? card : self.getCardInfoFallback(lookup, cardName, config)
      })
